...
def processar_transacoes_gui():
    def abrir_arquivo_entrada():
        arquivo_entrada = filedialog.askopenfilename(
            title="Selecione o arquivo de entrada", filetypes=[("CSV Files", "*.csv")]
        )
    ...
    def processar():
        ...
        try:
            ...
            messagebox.showerror(
                "Erro",
                "Valor mínimo inválido. Certifique-se de fornecer um número válido.",
            )
            return

        sucesso, mensagem_erro = processar_transacoes(
            arquivo_entrada, arquivo_saida, valor_minimo
        )

        if sucesso:
            messagebox.showinfo(
                "Concluído",
                f"Transações acima de ${valor_minimo} foram salvas em {arquivo_saida}.",
            )
        else:
    ...
    style.configure(
        "TLabel", foreground="#000000", background="#b3fb4f"
    )  # Texto preto, fundo verde
    style.configure(
        "TButton",
        background="#000000",
        foreground="#ffffff",
    ...
    ttk.Label(
        frame,
        text="Processar Transações",
        font=("Helvetica", 16, "bold"),
        anchor="center",
        background="#b3fb4f",
    ).grid(row=0, column=0, columnspan=3, pady=(0, 10))

    ttk.Label(frame, text="Arquivo de Entrada:").grid(
    ...
    ttk.Label(frame, text="Arquivo de Saída:").grid(
    ...
    btn_arquivo_saida = ttk.Button(
        frame, text="Salvar Como", command=abrir_arquivo_saida
    )
    btn_arquivo_saida.grid(row=2, column=2, sticky=tk.W, pady=(5, 5))

    ttk.Label(frame, text="Valor Mínimo:").grid(
        row=3, column=0, sticky=tk.W, pady=(5, 5)
    )
